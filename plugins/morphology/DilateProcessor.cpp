...
PluginMetadata DilateProcessor::metadata() const
{
...
QList<ParameterDescriptor> DilateProcessor::parameterDescriptors() const
{
    return {
...
void DilateProcessor::setParameter(const QString& id, const QVariant& value)
{
    if (id == "kernelSize") {
        int v = value.toInt();
        if (v % 2 == 0) v++;
...
QVariant DilateProcessor::parameter(const QString& id) const
{
...
void DilateProcessor::resetParameters()
{
    m_kernelSize = 3;
    m_kernelShape = 0;
    m_iterations = 1;
...
cv::Mat DilateProcessor::createKernel() const
{
...
ImageData DilateProcessor::process(const ImageData& input)
{
    if (input.isEmpty()) return input;

    cv::Mat kernel = createKernel();
    cv::Mat output;
    cv::dilate(input.mat(), output, kernel, cv::Point(-1, -1), m_iterations);
...
QJsonObject DilateProcessor::parameterState() const
{
    return {
...
void DilateProcessor::setParameterState(const QJsonObject& state)
{
